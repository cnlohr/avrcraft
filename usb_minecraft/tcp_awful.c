...
void TickTCP()
{
	...
	for( i = 0; i < TCP_SOCKETS; i++ )
	{
		...
		//Drop connections like a rock.  No resend.
		if( t->time_since_sent > TCP_TICKS_BEFORE_DEAD )
		{
//			sendstr( "X" );
			sendchr( 'T' );
			sendchr( 'X' );
...
{
	return (!TCPs[c].time_since_sent) && IsSendBufferFree();
}
